...
- credential, token, cookie, 필터링되지 않은 CLI output은 반환 금지
- `usageLines`는 Host가 허용한 `Current session`·`Current week (all models)` 행만 제어문자를 제거하고 길이를 제한해 제공
...
이 메서드가 없으면 plugin은 `getOverview()`에 포함된 account usage만 사용합니다. 구조화된 window가 없거나 일부만 있으면 `usageLines`에서 5시간·주간 슬롯을 보완합니다.
...
- plugin에는 account ID, 표시 label, 상태, 사용량, reset 시각과 허용된 사용량 행만 전달합니다.
- `usageLines`는 전체 원문이 아니며 `Current session`·`Current week (all models)` 두 종류만 포함합니다. Host는 ANSI·제어문자를 제거하고 행당 길이를 제한합니다.
- access token, refresh token, cookie, session file path와 나머지 command output을 전달하지 않습니다.
